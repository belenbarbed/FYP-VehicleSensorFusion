...
void Velodyne::Velodyne::project(Mat *frame_rgb, Mat projection_matrix, Rect frame, PointCloud<Point> *visible_points)
{
  ...
  for (PointCloud<Point>::iterator pt = point_cloud.points.begin(); pt < point_cloud.points.end(); pt++)
  {
    ...
    if (xy.inside(frame))
    {
      ...
      if(pt->range <= 10)
      {
        ...
        color[1] = 0;
        color[2] = 255;
        frame_rgb->at<Vec3b>(xy) = color;
      }
      else if(pt->range > 10 && pt->range <= 20)
      {
        Vec3b color;
        color[0] = 255;
        color[1] = 0;
