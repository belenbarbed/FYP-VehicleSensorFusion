...
double Velodyne::Velodyne::project(Mat *frame_rgb, Mat projection_matrix, Rect frame, Rect face, PointCloud<Point> *visible_points)
{
  Mat plane = cv::Mat::zeros(frame.size(), CV_32FC1);

  double closest_x, closest_y, closest_z, center_x, center_y;
  double closest_range = 1000;
  ...
  for (PointCloud<Point>::iterator pt = point_cloud.points.begin(); pt < point_cloud.points.end(); pt++)
  {
    ...
    if (xy.inside(frame))
    {
      ...
      double range = sqrt((xy.x - center_x) * (xy.x - center_x) + (xy.y - center_y) * (xy.y - center_y));
  ...
  double distance = sqrt(closest_x *closest_x + closest_y * closest_y + closest_z * closest_z);
  return distance;
}
